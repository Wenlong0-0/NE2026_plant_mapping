...
for i = 1:size(decay_coefficient,2)
    if new_t(i) > decay_start
        decay_coefficient(:,i) = exp(-(new_t(i)-decay_start)/0.8);
    end
end
...
for i = 1:electrode_number
    ...
    lead_field = (distance>electrode_position).*(distance<ref_e_position);
        % + 0.15*(distance>ref_e_position-0.3).*(distance<ref_e_position);
...
% exportgraphics(gcf, 'dipolar mid sim 2.pdf','ContentType',"vector");