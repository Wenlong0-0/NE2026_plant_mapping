...
infile = "mimosa touch top signal.txt";

sampling_rate = 100;     % Sampling rate, unit: Hz

% Line plot data range
line_start_index = 200;
line_end_index   = 1700;
...
line_ylim = [-100 50];   % Voltage range for line plot, unit: mV

% Imagesc interpolation settings
interp_factor = 100;     % Number of interpolated channels per original channel
trim_head_sec = 5;       % Seconds removed from the beginning
trim_tail_sec = 7;       % Seconds removed from the end
...
% Use selected time range for the line plot
data_line = data_4(line_start_index:line_end_index, :);

figure('Units', 'centimeters', 'Position', [5 5 line_fig_width line_fig_height]);

N_line = size(data_line, 1);
t_line = (1:N_line) / sampling_rate;

num_lines = size(data_line, 2);

% Use a reversed cool colormap for different channels
co = flipud(cool(num_lines));
colororder(co);

plot(t_line, data_line, 'LineWidth', line_width);
...
% Export the imagesc plot as a vector PDF
