...
all_speed = {};      % each sample: (num_channels-1)x1
all_amplitude = {};  % each sample: (num_channels)x1
...
for k = 1:nFiles
    ...
    % ---- Find max value (upward peak) per channel ----
    [amp_values, idx_max] = max(data, [], 1);  % 1×M
    time_of_peaks = t(idx_max);                % 1×M
...
% ===============================================================
% -------- Combine into matrices (columns = samples) ------------
% ===============================================================
max_len_speed = max(cellfun(@numel, all_speed));
max_len_amp   = max(cellfun(@numel, all_amplitude));

% Matrices are (vector_length × nFiles)
...
% ===============================================================
out_xlsx = "WaveSpeed_Amplitude_Matrices.xlsx";

% Add sample names as header row
speed_cell = [cellstr(sample_names); num2cell(speed_matrix)];
amp_cell   = [cellstr(sample_names); num2cell(amp_matrix)];

writecell(speed_cell, out_xlsx, 'Sheet', 'Speed_mm_per_s');
writecell(amp_cell,   out_xlsx, 'Sheet', 'Amplitude_mV');

fprintf("✅ Excel saved: %s\n", out_xlsx);

% ===============================================================
% -------- Mean and SD across samples (for plotting) ------------
% ===============================================================
% Columns are samples, so mean/std along dimension 2
